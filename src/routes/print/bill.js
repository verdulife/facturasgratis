...
export async function post(req, res) {
  ...
  const doc = new PDFDocument({
    ...
    margin: 0,
    info: {
      Title: `Factura_${numerationFormat(data.number, data.date.year)}_${data.client.legal_name}`,
    },
  });
...
${data.user.phone ? "t." + data.user.phone : ""} ${data.user.email ? "| e. " + data.user.email : ""
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="Factura_${numerationFormat(data.number, data.date.year)}_${data.client.legal_name}.pdf"`
  );

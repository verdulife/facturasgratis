...
import { numerationFormat, roundWithTwoDecimals, capitalize } from "../../lib/functions";
...
export async function post(req, res) {
  ...
  const doc = new PDFDocument({
    ...
    margin: 0,
    info: {
      Title: `${capitalize(doc_type)}_${numerationFormat(data.number, data.date.year)}_${data.client.legal_name}`,
    },
  });

  const req_regular = await fetch(req.headers.origin + "/fonts/fira-regular.woff");
  const regular = await req_regular.arrayBuffer();
  const req_medium = await fetch(req.headers.origin + "/fonts/fira-medium.woff");
...
${data.user.email ? "e. " + data.user.email : ""}`,
    ...
    doc.fontSize(8);
    doc.text(numerationFormat(data.number, data.date.year), mm(167.5), mm(31.2));
    ...
    doc.text(`${roundWithTwoDecimals(data.totals.base).toFixed(2)}${data.user.currency}`, mm(data.totals.ret > 0 ? 38.6 : 60.1), mm(237.3));
    doc.font(regular).text(`IVA ${data.user.iva}%`, mm(data.totals.ret > 0 ? 64.5 : 86.5), mm(237.3));
    doc.font(medium).text(`+${roundWithTwoDecimals(data.totals.iva).toFixed(2)}${data.user.currency}`, mm(data.totals.ret > 0 ? 79.5 : 101.1), mm(237.3));
    if (data.totals.ret > 0) {
      doc.font(regular).text(`IRPF ${data.user.ret}%`, mm(105.5), mm(237.3));
      doc.font(medium).text(`-${roundWithTwoDecimals(data.totals.ret).toFixed(2)}${data.user.currency}`, mm(122.5), mm(237.3));
    }
  ...
  const max_lines = 19;
  const chunkedLines = new Array(Math.ceil(data.items.length / max_lines)).fill().map(() => data.items.splice(0, max_lines));
  ...
  for (let c = 0; c < chunkedLines.length; c++) {
    ...
    for (let l = 0; l < chunk.length; l++) {
      ...
      doc.text(line.label, mm(34.8), mm(108) + jump, { width: mm(106), height: mm(6), ellipsis: true });
      doc.text(`${line.dto > 0 ? line.dto + "%" : "--"}`, mm(140.3), mm(108) + jump);
      doc.text(`${roundWithTwoDecimals(line.price).toFixed(2)}${data.user.currency}`, mm(150.5), mm(108) + jump);
      doc.text(`${roundWithTwoDecimals(line_total).toFixed(2)}${data.user.currency}`, mm(170.8), mm(108) + jump);
    }
  }

  res.statusCode = 200;
  res.setHeader("Content-type", "application/pdf");
  res.setHeader("Content-Disposition", `attachment; filename="${capitalize(doc_type)}_${numerationFormat(data.number, data.date.year)}_${data.client.legal_name}.pdf"`);
